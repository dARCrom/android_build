...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from dARCrom Github (http://github.com/dARCrom)." % device

repositories = []

page = 1
while not depsonly:
    request = Request("https://api.github.com/users/dARCrom/repos?page=%d" % page)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/dARCrom_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'dARCrom/%s already exists' % (repo_name)
            else:
    ...
    f = open('.repo/local_manifests/dARCrom_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for dARCrom/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: dARCrom/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "dARCrom/%s" % repo_name, "revision": "n7x" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/dARCrom.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the dARCrom repository list. If this is in error, you may need to manually add it to .repo/local_manifests/dARCrom_manifest.xml" % device
